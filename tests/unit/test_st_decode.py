...
from rosadmin.membership.solidarity_tech.fixtures import user_json


def _status(label):
    # The membership-status custom property is a select field: [{label, value}].
...
def test_good_standing_decodes():
    user = user_json(
        7,
        "kris@example.com",
        {"membership-status": _status("Member in Good Standing")},
    )
    m = decode_user(user)
    assert m.st_id == 7
...
def test_lapsed_decodes():
    user = user_json(8, "susie@example.com", {"membership-status": _status("Lapsed")})
    assert decode_user(user).standing is Standing.LAPSED


def test_missing_email_is_malformed():
    user = user_json(9, None, {"membership-status": _status("Member in Good Standing")})
...
def test_retired_tier_status_is_a_decode_error():
    user = user_json(
        10, "noelle@example.com", {"membership-status": _status("Lapsed Member")}
    )
