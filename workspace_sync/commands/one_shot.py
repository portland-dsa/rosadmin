...
class GroupSettings:
    ...
    def cloud_identity_name(self) -> str:
        if self.id is None:
            raise ValueError(
                "Please set the 'id' attribute after creating the group because the Cloud Identity API needs it"
            )
...
def _create_group(
    ...
    admin_service: DirectoryResource,
    ci_service: CloudIdentityResource,
    settings_resource: GroupssettingsResource,
) -> Group:
    group: Group = admin_service.groups().insert(body=settings.group_info).execute()
...
def _await_creation(
    settings: GroupSettings,
    ci_service: CloudIdentityResource,
    settings_service: GroupssettingsResource,
):
    """
...
def _await_member_add(
    settings: GroupSettings, admin_service: DirectoryResource, member_email: str
):
    result = (
        admin_service.members()
        .hasMember(groupKey=settings.email, memberKey=member_email)
        .execute()
    )
...
def one_shot_test_create_group(delete_at_end: bool = True) -> None:
    ...
    with alive_bar(10, title="test-create-group") as bar:
        ...
        admin_service: DirectoryResource = discovery.build(
            "admin", "directory_v1", credentials=creds
        )
        identity_service: CloudIdentityResource = discovery.build(
            "cloudidentity", "v1", credentials=creds
        )
        settings_service: GroupssettingsResource = discovery.build(
        ...
        ci_group = (
            identity_service.groups().get(name=settings.cloud_identity_name).execute()
        )
        bar()
